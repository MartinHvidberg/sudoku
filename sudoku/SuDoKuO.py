...
class SuDoKu(object):
    ...
    STPS = ['Free gifts', 'Crosshatching', 'Naked singles',
            'Locked Cand.']  # ToDo []: Consider tuple, or do we need to change it?

    def __init__(self, str_ini, lst_hardess=STPS):
        """ initialises a SuDoKu object """
        # ToDo: Overview of intended meaning of .m .v .p .rec, etc.
        self.hardness = lst_hardess
        self.m = [['0' for i in range(9)] for j in range(9)]  # Matrix: Empty (0 filled) matrix
        self.p = [[set() for i in range(9)] for j in range(9)]  # Pencil-marks: Empty (empty sets) pencil-matrix
        self.rec = list()  # Recorder: Track record of solving tactics steps
        if len(str_ini) == 0:
            pass  # No input string given, keeping empty sudoku in .m
        elif len(str_ini) == 81:
            log.info(f"Input: {str_ini}")
            self.i = str_ini  # Initialiser: Backup the original (init) fill, in string form
            try:
            ...
            if self.v:  # If it's valid, so far, check if it's solvable
                try:
                    ...
                    self.solution = [   # ToDo: Use one of the good solvers in other_solvers, like Roger-Wu !
                        [int(SuDoKuX.sudoku99_NEW_XXX_(str_ini)[j * 9 + i].replace('.', '0')) for i in range(9)] for j
                        in range(9)]  # Solve using brute force, to see if solution(s) exist
        ...
        for inp in [k, l, v - 1]:
            if not isinstance(inp, int) or inp < 0 or inp > 8:
                raise CustomException(
                    f"_set() takes three integers k, l and v: k and l must all be in [0..8], v must be in [1..9]. Received: {k, l, v}")
        self.m[k][l] = v
        self.p[k][
            l] = set()  # the k,l cell's own pencil marks. The cell now has a value, all other values are excluded.
    ...
    def _cps_areas(self):
        """ Return list of lists of cps (coordinate pairs),
        representing all rows and all cols and all boxs in the SuDoKu """
    ...
    def _slice9in3_vert(self, lst_in):
        ...
        if len(lst_in) == 9:
            return [[lst_in[0], lst_in[3], lst_in[6]], [lst_in[1], lst_in[4], lst_in[7]],
                    [lst_in[2], lst_in[5], lst_in[8]]]
        else:
            return []
        ...
        for i in range(9):
            for j in range(9):
                if self.get(i, j) != 0:
                    ...
                    self.p[i][j] = {1, 2, 3, 4, 5, 6, 7, 8, 9} - fixed
    ...
    def p_wipe_n_in_cps(self, num_n, lst_cps):
        ...
        if len(lst_cps) > 0:
            for cps in lst_cps:
                self.p[cps[0]][cps[1]] = self.p[cps[0]][cps[1]] - {num_n}
    ...
    def solved(self):
        ...
        for row in self.rows():
            empty += len([x for x in row if x == 0])  # ToDo: Write a test for this... Is it working as expected !?
        return empty == 0

    def validate(self):
        """ Return True if the SuDoKu is valid, otherwise returns False """
        bol_valid = True  # Until proven invalid
        ...
        for area in self.areas():  # check row, col and box doublets
            ...
            if len(a) != len(list(set(a))):
                bol_valid = False
                log.error(f"#201 > Area seems to have redundant values: {area}")
        log.info(f"validate() = {bol_valid}")
        return bol_valid
    ...
    def naked_singles(self):
        ...
        Something like: 'Only one pencil mark left in this cell'?. Check up on that..."""
    ...
    def show_small(self, matrix):
        ...
        def l(i):
            return ("|" + p(i, 0) + p(i, 1) + p(i, 2) + "|" + p(i, 3) + p(i, 4) + p(i, 5) + "|" + p(i, 6) + p(i, 7) + p(
                i, 8) + "|\n").replace('0', ' ')
